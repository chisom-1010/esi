...
export function CoursAnimes() {
  ...
  useEffect(() => {
    const fetchScores = async () => {
      ...
      try {
        // Appel de la fonction RPC 'scores_cours_animes_par_enseignant'
        const { data: scores, error: rpcError } = await supabase.rpc(
          "scores_cours_animes_par_enseignant",
        );
        ...
        console.error(
          "Erreur lors de la récupération des scores de cours animés:",
          err,
        );
        setError(
          `Échec du chargement des données des cours animés : ${err.message}`,
        );
      } finally {
        setIsLoading(false);
      }
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Cours Animés (Moyenne sur 20)</CardTitle>
        </CardHeader>
  ...
  if (data.length === 0) {
    return (
      <Card>
        ...
        <CardContent>
          Aucune donnée d'évaluation pour les cours animés disponible.
        </CardContent>
      </Card>
    );
  }
